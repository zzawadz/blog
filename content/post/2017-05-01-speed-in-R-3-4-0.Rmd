...
This version does not support this new functionality, so there is no difference between versions. This might be a bit confusing because it seems to be quite natural to add new observation to the end of a vector using `c(...)` function. However this cause massive loss of performance. It's better to avoid `c(...)` function in for loop!
